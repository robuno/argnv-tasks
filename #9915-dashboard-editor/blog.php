            ...
            while($row = mysqli_fetch_array($result))
                {
                    ...
                    <a href="blog-detail.php?id='.$row["id"].'"><h2>'.$row["title"].'</h2></a>
                        ...
                        <div icon="blog-post-text">
                            ...
                            $kelime = explode(" ",$row["content"]); // kaç kelime oldu bakalım..
                            $say = count($kelime);// sınır belirleyelim.. 
                            $sinir = 40;// eğer metindeki kelime sayısı sınırımızdan az ya da eşit ise yarısını keselim..
                            if($say <= $sinir) { $kes = $say*50/100; } // sınırdan çok ise, sınır belirlediğimiz kelime sayısı olsun..
                            else { $kes = $sinir; } // yazıyı ekrana basalım..
                            
                            for($i = 0; $i <= $kes; $i++)
                            { echo strip_tags($kelime[$i]).' ' ; }
                            // devam yazısı..
